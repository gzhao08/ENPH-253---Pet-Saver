 ...
 */
void ClawManager::begin(ClawBase* base, ClawVerticalStage* vertical, ClawArm* arm, ClawGrabber* grab, Basket* basket) {
    this->base = base;
 ...
 */
void sense(bool height, bool direction) {
    //reset height
    this->vertical->setPosition(this->vertical->RESET_HEIGHT);
    this->verticalPos = this->vertical->RESET_HEIGHT;
 ...
 */
void windowDrop(bool direction) {
...
/**
 * releases zipline mechanism from down position beside basket
 */
void basketRelease() {
    this->basket->release();
}  
